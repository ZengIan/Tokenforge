...
def test_weights_scale_with_quant():
    fp16 = estimate(_req("H20-3e (141G)", 8, quant="FP16"))
    int4 = estimate(_req("H20-3e (141G)", 8, quant="INT4"))
    # INT4 weights ~ 1/4 of FP16 (0.5 vs 2 bytes), allowing group overhead
    assert int4.memory.weights_gb < fp16.memory.weights_gb * 0.3


def test_higher_bandwidth_improves_decode():
    # H20-3e has higher bandwidth than A100 -> lower TPOT (faster decode)
    h20 = estimate(_req("H20-3e (141G)", 8, concurrency=16))
    a100 = estimate(_req("A100 SXM (80G)", 8, concurrency=16))
    assert h20.tpot_ms < a100.tpot_ms


def test_concurrency_increases_tps():
    low = estimate(_req("H100 SXM (80G)", 8, concurrency=1))
    high = estimate(_req("H100 SXM (80G)", 8, concurrency=32))
    assert high.tps > low.tps


def test_oversized_model_does_not_fit():
    # 72B FP16 weights (~134GB) on a single small card cannot fit
    r = estimate(_req("A100 SXM (80G)", 1, quant="FP16", concurrency=1))
    assert r.fits is False


def test_fp8_warns_on_ampere():
    r = estimate(_req("A100 SXM (80G)", 8, quant="FP8"))
